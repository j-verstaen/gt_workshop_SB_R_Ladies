...
thanksgiving_pie %>% 
  ...
  tab_source_note(source_note = "Source: FiveThirtyEight survey from 2015",
                  source_note = "Table created 11/18/20 for R-Ladies SB")  
...
There is much more you can do in regards to formating the table/ columns to fit your needs. I highly recommend checking out XX (the official gt r studio thing) to see what options are avaliable to you.
...
thanksgiving_pie %>% 
  ...
  ## NOTE: even once you have changed the column labels, that column name in the df is still the original name. So, when using functions that refer to the columns moving forward, we will still need to refer to the original column labels. This is kind of nice becuase it gives you freedom in regards to labeling
...
## Lets organize the rows in descending order for survery_percentage. Remember, to create a gt table we start with a regular data frame that can be wrangles anyway we want before using gt() to convert it into a table. Here we will arrange the df before making it ito a table
...
thanksgiving_pie %>% 
  ...
  ## Lets say we really want to highlight the differences in counts for each pie flavor. We will need to make sure column values are numeric if they should be but they aren't you can reformat it with fmt_numeric()
...
## Step 7: gt will keep grouping varibales and facet accordingly
...
* summary_rows() will calcualte a row of wahtaver specificied summary statistis. requires some setting up of identifying grouped variables.
